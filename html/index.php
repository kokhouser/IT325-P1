...
<style type="text/css">
 ...
 #name, #password, #mail, #confirm_password {
    width: 250px;
    margin-bottom: 15px;
...
<?php
 ...
 if(isset($_POST['submit']))
 {
  ...
  $mail=$_POST['mail'];
  $q=mysql_query("select * from userLogin where username='".$name."' or email='".$mail."' ") or die(mysql_error());
  $n=mysql_fetch_row($q);
  if($n>0)
  {
   $er='The username '.$name.' or mail '.$mail.' is already present in our database';
  }
  else
  {
   $insert=mysql_query("insert into userLogin values('".$name."','".$username."','".$password."','".$mail."','False')") or die(mysql_error());
   if($insert)
   {
    $er='User has succesfully registered!';
   }
   else
   {
...
<h1>Registration Form</h1>
     ...
     <form action="#" method="post">
      <table id="tbl" align="center">
       ...
       <tr><td>Confirm Password:</td><td><input type="password" name="confirm_password" id="confirm_password"></td></tr>
